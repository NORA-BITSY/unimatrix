...
class BlockchainService {
  ...
  async getBalance(address: string, networkName: string = 'ethereum'): Promise<WalletBalance | null> {
    try {
  ...
  async getTransaction(txHash: string, networkName: string = 'ethereum'): Promise<Transaction | null> {
    try {
      const provider = this.providers.get(networkName);
      if (!provider) {
        throw new Error(`Network ${networkName} not available`);
  ...
  async getTokenInfo(tokenAddress: string, networkName: string = 'ethereum'): Promise<TokenInfo | null> {
    try {
      const erc20Abi = [
  ...
  async getGasPrice(networkName: string = 'ethereum'): Promise<string | null> {
    try {
      const provider = this.providers.get(networkName);
      if (!provider) {
        throw new Error(`Network ${networkName} not available`);
  ...
  async getBlockNumber(networkName: string = 'ethereum'): Promise<number | null> {
    try {
      const provider = this.providers.get(networkName);
      if (!provider) {
        throw new Error(`Network ${networkName} not available`);
  ...
  async checkNetworkHealth(networkName: string): Promise<boolean> {
    try {
