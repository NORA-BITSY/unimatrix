...
export async function aiRoutes(fastify) {
    ...
    fastify.post('/chat', {
        ...
        try {
            ...
            const response = await aiService.generateChat(messages, validatedData.provider, {
            ...
            // Save conversation
            if (!conversationId) {
                conversationId = await aiService.saveConversation(userId, messages, `Chat ${new Date().toLocaleDateString()}`, response.model);
            }
            else {
    ...
    fastify.post('/conversations', {
        ...
        try {
            ...
            const userId = request.user.id;
            const conversationId = await aiService.saveConversation(userId, [], validatedData.title || 'New Conversation', validatedData.model);
            await reply.send({
                success: true,
                data: { conversationId },
            });
    ...
    fastify.post('/generate', {
        ...
        try {
            const { prompt, provider = 'openai', model, temperature, maxTokens } = request.body;
